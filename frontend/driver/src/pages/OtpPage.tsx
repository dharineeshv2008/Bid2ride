...
export const OtpPage: React.FC = () => {
  ...
  const phone = location.state?.phone || '+919876543211';
  ...
  return (
    <div>
      <button
        ...
        className="inline-flex items-center gap-1 text-xs font-semibold text-emerald-400 hover:text-emerald-300 mb-4"
      >
        <ArrowLeft className="w-4 h-4" />
        Change Phone Number
      </button>

      <h2 className="text-2xl font-bold text-white mb-1 font-display">Verification Code</h2>
      <p className="text-slate-400 text-sm mb-6">
        Code sent to <span className="font-semibold text-slate-200">{phone}</span>.
      </p>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div>
          ...
          <input
            ...
            className="w-full px-4 py-3 rounded-2xl bg-slate-900 border border-slate-800 focus:outline-none focus:ring-2 focus:ring-emerald-500/30 text-white font-medium transition-all disabled:opacity-50"
          />
        </div>
        ...
        <div className="flex items-center justify-between text-sm px-1">
          {timer > 0 ? (
            <span className="text-slate-400">
              Resend code in <span className="font-semibold text-slate-200">{timer}s</span>
            </span>
          ) : (
            <button
              type="button"
              ...
              disabled={isSubmitting}
              className="inline-flex items-center gap-1.5 font-bold text-emerald-400 hover:text-emerald-300 transition-colors disabled:opacity-50"
            >
